...
def create_workflow(Kconfig,args):
    ...
    if cur_iter==0:
      ...
      pre_proc_task2.copy_input_data = ['$SHARED/%s > %s/%s' % (args.Kconfig,combined_path, args.Kconfig),
                                     '$SHARED/run-tica-msm3.py > %s/run-tica-msm3.py' % combined_path,
    ...
    while(cur_iter <  int(Kconfig.num_iterations)):
	  ...
	  pre_exec_arr = [ 'module swap PrgEnv-cray PrgEnv-gnu','module load bwpy/1.2.5','module add bwpy-mpi', 'module add fftw/3.3.4.10', 'module add cray-netcdf', 'module add cudatoolkit/9.1.85_3.10-1.0502.df1cc54.3.1', 'module add cmake/3.1.3', 'module unload darshan xalt','export CRAYPE_LINK_TYPE=dynamic', 'export CRAY_ADD_RPATH=yes', 'export FC=ftn', 'source /projects/sciteam/bamm/hruska/vpy4/bin/activate', 
        ...
        if str(Kconfig.strategy)!='extend':
          ...
          ana_task.arguments = ['run-tica-msm3.py', '--path',combined_path,'--n_select', str(num_replicas),'--cur_iter',str(cur_iter), '--Kconfig', str(args.Kconfig), '--ref', str(Kconfig.md_reference), '>', 'analyse.log']
          ...
          ana_task.link_input_data = ['$SHARED/run-tica-msm3.py > run-tica-msm3.py', '$SHARED/%s > %s'%(args.Kconfig,args.Kconfig)]
    ...
    try:
        ...
        if Kconfig.md_use_xml=='yes':
          shared_data_all=shared_data_all+['%s/system-5.xml' % Kconfig.md_dir,
                                          ...
                                          '%s/run-tica-msm3.py' % Kconfig.helper_scripts]
        else:
