    ...
    while(cur_iter <  int(Kconfig.num_iterations)):
        ...
        for sim_num in range(min(int(Kconfig.num_parallel_MD_sim),int(Kconfig.num_replicas))):
            ...
            if Kconfig.use_gpus=='False':
              ...
              sim_task.cpu_reqs = sim_task.cpu_reqs = { 'processes': int(Kconfig.num_CUs_per_MD_replica),
                                    'process_type': None,
                                    'threads_per_process': 1,
                                    'thread_type': None
                                  }
               #int(Kconfig.num_CUs_per_MD_replica)
...
if __name__ == '__main__':
    ...
    try:
        ...
        shared_data_all = [Kconfig.md_input_file,
                            Kconfig.md_run_file,
